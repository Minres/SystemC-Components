...
set CMAKE_OPTS=-DCMAKE_GENERATOR_PLATFORM=x64 -DCMAKE_CXX_STANDARD=%CXX_STD%
...
cmake -S fmt -B build/fmt %CMAKE_OPTS% -DCMAKE_INSTALL_PREFIX=%SCC_HOME%
cmake --build build/fmt -j 10 --config Release --target install
@REM ############################################################################################
@REM build_spdlog
@REM ############################################################################################
if NOT EXIST "spdlog" tar xzvf spdlog_1.9.2.tar.gz
cmake -S spdlog -B build/spdlog %CMAKE_OPTS% -DCMAKE_INSTALL_PREFIX=%SCC_HOME% -DSPDLOG_FMT_EXTERNAL_HO=ON
...
@REM build_zlib
@REM ############################################################################################
if NOT EXIST "zlib-1.3" tar xzvf zlib-1.3.tar.gz
cmake -S zlib-1.3 -B build/zlib %CMAKE_OPTS% -DCMAKE_INSTALL_PREFIX=%SCC_HOME%
cmake --build build/zlib -j 10 --config Release --target install
@REM ############################################################################################
@REM build_systemc
@REM ############################################################################################
if NOT EXIST "systemc" tar xzvf systemc_2.3.4.tar.gz
cmake -S systemc -B build/systemc %CMAKE_OPTS% -DCMAKE_INSTALL_PREFIX=%SYSTEMC_HOME%
