...
class _scv_tr_db_core {
 public:
  _scv_tr_db_core(const char* recording_file_name,
...
class _scv_tr_stream_core {
 ...
 public:
  ...
  _scv_tr_stream_core(_scv_tr_db_core* _scv_tr_db_core_p,
                      scv_tr_stream* scv_tr_stream_p, const char* name,
...
class _scv_tr_generator_core {
 ...
 public:
  ...
  _scv_tr_generator_core(_scv_tr_stream_core* scv_tr_stream_core_p,
                         scv_tr_generator_base* scv_tr_generator_base_p,
                         const char* name, const char* begin_attribute_name,
...
_scv_tr_generator_core::_scv_tr_generator_core(
  ...
  if ((scv_tr_stream_core_p == NULL) || (scv_tr_generator_base_p == NULL) ||
...
template <class object_class_parameter_t, typename callback_reason_parameter_t,
          typename callback_function_parameter_t>
static void process_callbacks(const object_class_parameter_t& obj,
...
static void process_record_attribute_callbacks(
    const scv_tr_handle& obj, const char* attribute_name,
    _scv_tr_callback_list* callback_list_p,
    ...
    for (; i != callback_list_p->end(); i++) {
      if ((*i)->callback_fp == NULL) {
        ...
        scv_tr_handle::callback_record_attribute_function* my_callback_fp =
            (scv_tr_handle::
                 callback_record_attribute_function*)((*i)->callback_fp);
...
static void process_relation_callbacks(
    const scv_tr_handle& obj, const scv_tr_handle& obj_2,
    _scv_tr_callback_list* callback_list_p,
...
scv_tr_db::scv_tr_db(const char* recording_file_name,
  ...
  this->_scv_tr_db_core_p =
      new _scv_tr_db_core(recording_file_name, _time_unit);
  ...
  process_callbacks<scv_tr_db, scv_tr_db::callback_reason,
...
scv_tr_db::~scv_tr_db() {
  ...
  if (d >= 0) {
    cout << "scv_tr debug: entering scv_tr_db dtor, name = " << this->get_name()
         << endl;
  }
...
scv_tr_db::callback_h scv_tr_db::register_class_cb(
    scv_tr_db::callback_function* cbf, void* user_data_p) {
...
scv_tr_stream::scv_tr_stream(const char* full_stream_name,
                             const char* kind_name, scv_tr_db* scv_tr_db_p) {
#ifdef scv_tr_TRACE
  ...
  this->_scv_tr_stream_core_p = new _scv_tr_stream_core(
      scv_tr_db_p->_scv_tr_db_core_p, this, full_stream_name, kind_name);

  process_callbacks<scv_tr_stream, scv_tr_stream::callback_reason,
...
scv_tr_stream::callback_h scv_tr_stream::register_class_cb(
    scv_tr_stream::callback_function* cbf, void* user_data_p) {
...
scv_tr_handle::~scv_tr_handle() {
  ...
  if (this->_scv_tr_handle_core_p != NULL) {
    ...
    if (this->_scv_tr_handle_core_p->ref_count == 1) {
      process_callbacks<scv_tr_handle, scv_tr_handle::callback_reason,
...
void scv_tr_handle::_record_attribute(const char* attribute_name,
                                      const scv_extensions_if* my_exts_p) {
  const char* tmp_attribute_name =
      attribute_name ? attribute_name : "<anonymous>";

  process_record_attribute_callbacks(
      *this, tmp_attribute_name,
      _scv_tr_handle_core::callback_record_attribute_list_p, my_exts_p);
}

// ----------------------------------------------------------------------------

scv_tr_handle::callback_h scv_tr_handle::register_class_cb(
    scv_tr_handle::callback_function* cbf, void* user_data_p) {
...
scv_tr_handle::callback_h scv_tr_handle::register_record_attribute_cb(
    scv_tr_handle::callback_record_attribute_function* cbf, void* user_data_p) {
...
scv_tr_handle::callback_h scv_tr_handle::register_relation_cb(
    scv_tr_handle::callback_relation_function* cbf, void* user_data_p) {
...
bool scv_tr_handle::add_relation(scv_tr_relation_handle_t relation_handle,
  ...
  process_relation_callbacks(*this, other_transaction,
...
void scv_tr_handle::_end_transaction(const scv_extensions_if* exts_p,
                                     const sc_time& end_sc_time) const {
  this->get_scv_tr_generator_base()._end_transaction(*this, exts_p,
                                                     end_sc_time);
}
...
scv_tr_handle scv_tr_generator_base::_begin_transaction(
    const scv_extensions_if* ext_p, const sc_time& begin_sc_time,
    scv_tr_relation_handle_t relation_handle,
  ...
  tp._scv_tr_handle_core_p->my_id =
      this->_scv_tr_generator_core_p->my_scv_tr_stream_core_p
          ->my_scv_tr_db_core_p->global_transaction_id++;
...
scv_tr_generator_base::scv_tr_generator_base(const char* name, scv_tr_stream& s,
                                             const char* begin_attribute_name,
                                             const char* end_attribute_name) {
  this->_scv_tr_generator_core_p =
      new _scv_tr_generator_core(s._scv_tr_stream_core_p, this, name,
...
void scv_tr_generator_base::_process_callbacks() {
  ...
  process_callbacks<scv_tr_generator_base,
                    scv_tr_generator_base::callback_reason,
                    scv_tr_generator_base::callback_function>(
      *this, _scv_tr_generator_core::callback_list_p,
...
scv_tr_db::callback_h scv_tr_generator_base::register_class_cb(
    callback_function* cbf, void* user_data_p) {
...
const char* scv_tr_db::get_relation_name(
  ...
  return this->_scv_tr_db_core_p->relation_by_handle_map[relation_handle]
      .c_str();
}
