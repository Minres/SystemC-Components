...
inline static int s_add_callback(_scv_dynamic_data* data, _scv_dynamic_data::callback_base* c) {
    if(!data) {
...
inline static void s_remove_callback(_scv_dynamic_data* data, scv_extensions_if::callback_h id) {
    if(id != -1) {
        if(!data) {
            _scv_message::message(_scv_message::INTROSPECTION_INVALID_DYNAMIC_EXTENSIONS, "callbacks");
            return;
...
s_select_callback(_scv_dynamic_data* data, scv_extensions_if::callback_h id) {
    if(!data) {
        _scv_message::message(_scv_message::INTROSPECTION_INVALID_DYNAMIC_EXTENSIONS, "callbacks");
...
// ----------------------------------------
template <typename T> class _scv_extension_callbacks_base : public _SCV_INTROSPECTION_BASE {
...
public:
    ...
    virtual void remove_cb(scv_extensions_if::callback_h id) { s_remove_callback(this->_get_dynamic_data(), id); }

    // void trigger_value_change_cb();
    // this is implemented as non-virtual in extension "util" for efficiency.
};

template <typename T> class scv_extension_callbacks : public _scv_extension_callbacks_base<T> {
public:
    virtual scv_extensions_if::callback_h _register_cb(scv_extensions_if::callback_base* c) {
...
// ----------------------------------------
template <typename T, int N> class scv_extension_callbacks<T[N]> : public _SCV_INTROSPECTION_BASE2 {
public:
...
// ----------------------------------------
template <typename T> class scv_extension_callbacks<T*> : public _SCV_INTROSPECTION_BASE1 {
public:
    scv_extension_callbacks() {}
    virtual ~scv_extension_callbacks() {
        if(this->_has_dynamic_data() && !this->get_parent())
...
// ----------------------------------------
class _scv_extension_callbacks_enum : public _SCV_INTROSPECTION_BASE_ENUM {
public:
...
#define _SCV_EXT_CALLBACKS_FC_D(basic_type, type_id)                                                                   \
    class _scv_extension_callbacks_##type_id : public scv_extension_rw<basic_type> {                                   \
    public:                                                                                                            \
        ...
        /* this is implemented as non-virtual in extension "util" for efficiency. */                                   \
                                                                                                                       ...
                                                                                                                       \
    template <> class scv_extension_callbacks<basic_type> : public _scv_extension_callbacks_##type_id {};
...
#define _SCV_EXT_CALLBACKS_FC_N_D(basic_type)                                                                          \
    template <int N> class scv_extension_callbacks<basic_type<N>> : public scv_extension_rw<basic_type<N>> {           \
    public:                                                                                                            \
        scv_extension_callbacks() {}                                                                                   \
        virtual ~scv_extension_callbacks() {                                                                           \
            if(this->_has_dynamic_data() && !this->get_parent())                                                       \
