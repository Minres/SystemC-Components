...
namespace scc {
template <typename T> class sc_attribute_randomized : public sc_core::sc_attribute<T> {
public:
    ...
    template <typename U = T> resolvedType<std::is_signed<T>::value, U> get_value() {
        if(this->value < 0)
    ...
    template <typename U = T> resolvedType<std::is_unsigned<T>::value, U> get_value() { return this->value; }
};
} // namespace scc
/** @} */ // end of scc-sysc
