...
struct fst_trace {
    ...
    fst_trace(std::string const& nm, trace_type type, unsigned bits): name{nm}, bits{bits}, type{type}{}
...
struct fst_trace_enum : public fst_trace {
    fst_trace_enum( const unsigned int& object_,
    ...
    void record(void* os) override {
        fstWriterEmitValueChange64(os, fst_hndl, bits, old_val);
    }

    void update_and_record(void* os) override {update(); record(os);};
...
inline void fst_trace_t<T, OT>::record(void *m_fst) {
    if(sizeof(T)<=4)
...
void fst_trace_file::init() {
    ...
    for(auto& e:all_traces){
        ...
        e.trc->fst_hndl = fstWriterCreateVar(m_fst, e.trc->type==trace::REAL?FST_VT_VCD_REAL:FST_VT_VCD_WIRE,
                FST_VD_IMPLICIT, e.trc->bits, sig_name.c_str(), e.trc->is_alias?alias_it->second:0);
        if(!e.trc->is_alias)
