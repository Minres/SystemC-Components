...
struct vcd_pull_trace_file::Trace {
    Trace(std::string const& nm, trace_type t, unsigned bits): name{nm}, type{t}, bits{bits}{}
...
namespace {
struct vcd_trace_enum : public vcd_pull_trace_file::Trace {
    vcd_trace_enum( const unsigned int& object_,
            const std::string& name, const char** literals)
    : vcd_pull_trace_file::Trace( name, WIRE, get_bits(literals))
    , act_val( object_ )
    , old_val( object_ )
    , literals{literals}
    {}
...
struct vcd_trace_t : public vcd_pull_trace_file::Trace {
    vcd_trace_t( const T& object_,
            const std::string& name)
    : vcd_pull_trace_file::Trace( name, get_type(), get_bits())
    , act_val( object_ )
    , old_val( object_ )
    {}
...
template<> void vcd_trace_t<sc_core::sc_time, sc_core::sc_time>::record(FILE* os){
    vcdEmitValueChange64(os, fst_hndl, bits, old_val.value());
}
 ...
 * sc_dt::sc_fxval
 */
template<> trace_type vcd_trace_t<sc_dt::sc_fxval, sc_dt::sc_fxval>::get_type(){
    return REAL;
}
 ...
 * sc_dt::sc_fxval_fast
 */
template<> trace_type vcd_trace_t<sc_dt::sc_fxval_fast, sc_dt::sc_fxval_fast>::get_type(){
    return REAL;
}
 ...
 */
template<> trace_type vcd_trace_t<sc_dt::sc_fxnum, sc_dt::sc_fxval>::get_type(){
    return REAL;
}
 ...
 * sc_dt::sc_fxnum_fast
 */
template<> trace_type vcd_trace_t<sc_dt::sc_fxnum_fast, sc_dt::sc_fxval_fast>::get_type(){
    return REAL;
}
...
struct vcd_scope_stack {
    void add_trace(vcd_pull_trace_file::Trace *trace){
...
private:
    void add_trace_rec(std::vector<std::string>::iterator beg, std::vector<std::string>::iterator const& end, vcd_pull_trace_file::Trace *trace){
    ...
    void print_variable_declaration_line(FILE* os, const char* scoped_name, vcd_pull_trace_file::Trace* trc){
        char buf[2000];
        switch(trc->bits){
        case 0:{
            std::stringstream ss;
    ...
    std::vector<std::pair<std::string,vcd_pull_trace_file::Trace*> > m_traces{0};
...
template<typename T, typename OT=T>
bool changed(vcd_pull_trace_file::Trace* trace) {
