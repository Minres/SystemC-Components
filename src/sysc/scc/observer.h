...
#define DECL_REGISTER_METHOD_A(tp)                                                                                                         \
    inline observer::notification_handle* observe(observer* obs, tp const& o, std::string const& nm) { return obs->observe(o, nm); }
#if(SYSTEMC_VERSION >= 20171012)
...
template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_core::sc_signal_in_if<T>& object, const char* name) {
...
template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_core::sc_signal_in_if<T>& object, const std::string& name) {
    sc_trace(tf, object, name.c_str());
}

template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_core::sc_in<T>& port, char const* name) {
...
template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_core::sc_in<T>& port, const std::string& name) {
    sc_trace(tf, port, name.c_str());
}

template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_core::sc_inout<T>& port, char const* name) {
    const sc_core::sc_signal_in_if<T>* iface = 0;
    if(sc_core::sc_get_curr_simcontext()->elaboration_done())
        iface = dynamic_cast<const sc_core::sc_signal_in_if<T>*>(port.get_interface());
    if(iface)
...
template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_core::sc_inout<T>& port, const std::string& name) {
    sc_trace(tf, port, name.c_str());
}

#define DEFN_TRACE_FUNC_REF_A(tp)                                                                                                          \
    inline void sc_trace(sc_core::sc_trace_file* tf, const tp& object, const std::string& name) { sc_core::sc_trace(tf, object, name); }
