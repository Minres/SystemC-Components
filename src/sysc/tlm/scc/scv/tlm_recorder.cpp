...
namespace scv {
namespace {
const std::array<char const*, 3> cmd2char{{"READ", "WRITE", "IGNORE"}};
const std::array<char const*, 7> resp2char{{"OK", "INCOMPLETE", "GENERIC_ERROR", "ADDRESS_ERROR", "COMMAND_ERROR", "BURST_ERROR", "BYTE_ENABLE_ERROR"}};
const std::array<char const*, 3> gp_option2char{{"MIN_PAYLOAD", "FULL_PAYLOAD", "FULL_PAYLOAD_ACCEPTED"}};
const std::array<char const*, 5> phase2char{{"UNINITIALIZED_PHASE", "BEGIN_REQ", "END_REQ", "BEGIN_RESP", "END_RESP"}};
const std::array<char const*, 4> dmi2char{{"DMI_ACCESS_NONE", "DMI_ACCESS_READ", "DMI_ACCESS_WRITE", "DMI_ACCESS_READ_WRITE"}};
const std::array<char const*, 3> sync2char{{"ACCEPTED","UPDATED","COMPLETED"}};
}
void record(scv_tr_handle& handle, tlm::tlm_generic_payload& o){
    handle.record_attribute("trans.address",o.get_address());
    handle.record_attribute("trans.cmd",cmd2char[o.get_command()]);
    handle.record_attribute("trans.data_ptr",o.get_data_ptr());
    handle.record_attribute("trans.data_length",o.get_data_length());
    handle.record_attribute("trans.response",resp2char[o.get_response_status()]);
    ...
    handle.record_attribute("trans.gp_option",o.get_gp_option());
    ...
    for(auto& ext : tlm_extension_recording_registry<TYPES>::inst().get())
        if(ext)
...
void record(scv_tr_handle& handle, tlm::tlm_sync_enum o){
    handle.record_attribute("tlm_sync",sync2char[o]);
}
void record(scv_tr_handle& handle, tlm::tlm_dmi& o){
    ...
    handle.record_attribute("trans.granted_access",dmi2char[o.get_granted_access()]);
    handle.record_attribute("trans.read_latency",o.get_start_address());
    handle.record_attribute("trans.write_latency",o.get_write_latency());
