...
class abstract_bitfield {
 public:
  constexpr abstract_bitfield(std::string name, size_t bitOffset,
 ...
 */
template <typename datatype_t>
class bitfield_register : public sc_core::sc_object,
                          public scc::resource_access_if {
 public:
  /**
   ...
   */
  constexpr bitfield_register(sc_core::sc_module_name name, size_t offset,
  ...
  bool write(const uint8_t* data, std::size_t length, uint64_t offset = 0,
    ...
    std::copy(data, data + length,
              reinterpret_cast<uint8_t*>(&valueToWrite) + offset);
    for (auto&& bitfield : bitfields) {
      if (bitfield.get().affected(offset, length)) {
        ...
        valueToWrite = (valueToWrite & ~mask) |
                       ((bits << bitfield.get().bitOffset) & mask);
      }
    }
    if (writeCallback) writeCallback(*this, valueToWrite);
  ...
  bool read(uint8_t* data, std::size_t length, uint64_t offset = 0,
    ...
    for (auto&& bitfield : bitfields) {
      if (bitfield.get().affected(offset, length)) {
        ...
        auto mask = bitfield.get().mask();
        result = (result & ~mask) |
                 ((bitfieldValue << bitfield.get().bitOffset) & mask);
      }
    }
    if (readCallback) readCallback(*this, result);
    auto begin = reinterpret_cast<const uint8_t*>(&result) + offset;
    std::copy(begin, begin + length, data);
    return true;
  }

  bool write_dbg(const uint8_t* data, std::size_t length,
  ...
  bool read_dbg(uint8_t* data, std::size_t length,
   ...
   * not work.
   */
  void setWriteCallback(std::function<void(bitfield_register<datatype_t>&,
   ...
   * Callback is called after the individual bitfields.
   */
  void setReadCallback(std::function<void(const bitfield_register<datatype_t>&,
                                          datatype_t& result)>
                           callback) {
 ...
 protected:
  ...
  std::function<void(bitfield_register<datatype_t>&, datatype_t&)>
      writeCallback;
  std::function<void(const bitfield_register<datatype_t>&, datatype_t&)>
      readCallback;
  std::vector<std::reference_wrapper<abstract_bitfield<datatype_t>>> bitfields;
};

template <typename datatype_t>
class bitfield : public abstract_bitfield<datatype_t> {
 public:
  ...
  bitfield(bitfield_register<datatype_t>& reg, std::string name,
                     size_t bitOffset, size_t bitSize, std::string urid,
                     Access access = RW)
      : reg{reg},
        abstract_bitfield<datatype_t>{std::move(name), bitOffset, bitSize,
                                      std::move(urid)},
        access{access} {
  ...
  void write(datatype_t& valueToWrite) override {
    if (writeCallback) writeCallback(*this, valueToWrite);
    if (access == ReadOnly) valueToWrite = get();
  }
  ...
  void put(datatype_t value) {
    reg.put((reg.get() & ~mask()) | ((value << bitOffset) & mask()));
  }
  ...
  void setReadCallback(
      std::function<datatype_t(const bitfield<datatype_t>&)> callback) {
    readCallback = std::move(callback);
  }
...
class tlm_target_bfs_register_base : public sc_core::sc_object,
 ...
 public:
  ...
  bitfield_register<uint32_t>& getRegister(const std::string& name) {
    auto found =
        std::find_if(asDerived().registers.begin(), asDerived().registers.end(),
                     [name](const bitfield_register<uint32_t>& reg) {
   ...
   */
  bitfield<uint32_t>& getBitfieldByName(const std::string& regname,
                                        const std::string& name) {
   ...
   * If no matching bitfield is found a FATALERROR is generated.
   */
  bitfield<uint32_t>& getBitfield(const std::string& regname,
                                  const std::string& name,
                                  const std::string& urid) {
    if (use_URID) {
...
#define BITFIELD_ARRAY_ELEMENT(z, i, elem)                              \
  BOOST_PP_COMMA_IF(i) {                                                \
    getRegister(BOOST_PP_TUPLE_ELEM(0, elem)),                          \
        BOOST_PP_TUPLE_ELEM(1, elem) BOOST_PP_STRINGIZE(i),             \
        BOOST_PP_TUPLE_ELEM(2, elem) + i *BOOST_PP_TUPLE_ELEM(3, elem), \
        BOOST_PP_TUPLE_ELEM(3, elem),                                   \
        BOOST_PP_TUPLE_ELEM(4, elem) BOOST_PP_STRINGIZE(i)              \
...
#define BITFIELD_ARRAY(regname, basename, offset, size, uridbase, count) \
  BOOST_PP_REPEAT(count, BITFIELD_ARRAY_ELEMENT,                         \
                  (regname, basename, offset, size, uridbase, ))
 ...
 */
#define BITFIELD_ARRAY_POSTFIX(regname, basename, offset, size, uridbase, \
                               uridpostfix, count)                        \
  BOOST_PP_REPEAT(count, BITFIELD_ARRAY_ELEMENT,                          \
                  (regname, basename, offset, size, uridbase, uridpostfix))
...
#define REGISTER_ARRAY_ELEMENT(z, i, elem)                             \
  BOOST_PP_COMMA_IF(i) {                                               \
    ...
    BOOST_PP_STRINGIZE(i),                                             \
        BOOST_PP_TUPLE_ELEM(1, elem) + i* BOOST_PP_TUPLE_ELEM(2, elem) \
  }
