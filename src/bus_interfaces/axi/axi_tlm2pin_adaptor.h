...
public:
    ...
    tlm::scc::target_mixin<tlm::scc::scv4tlm::tlm_rec_target_socket<BUSWIDTH, axi::axi_protocol_types>, axi::axi_protocol_types>
    ...
    if(!resetn_i.read()) { // active-low reset
        ...
        if(r_valid_i.read()) {
            ...
            if(read_trans->phase == tlm::END_REQ || read_trans->phase == axi::BEGIN_PARTIAL_RESP) {
                ...
                SCCTRACE(SCMOD) << read_trans->phase << " bw trans " << std::hex << read_trans->payload << std::dec
                                << " (axi_id:" << id << ")";
        ...
        if(b_valid_i.read()) {
            ...
            if(write_trans->phase == tlm::END_REQ) {
                ...
                SCCTRACE(SCMOD) << write_trans->phase << " bw trans " << std::hex << write_trans->payload << std::dec
                                << " (axi_id:" << id << ")";
