...
template <typename CFG>
inline tlm::tlm_sync_enum axi::pin::ace_target<CFG>::nb_transport_bw(payload_type& trans, phase_type& phase,
        sc_core::sc_time& t) {
...
template <typename CFG> inline void axi::pin::ace_target<CFG>::ar_t() {
    ...
    while(true) {
        ...
        //        ardomain = this->ar_domain->read().to_uint();
            }
            ...
            ext->set_size(arsize);
            /*TBD?? read other signals and pack into ext
             * */
       //     ext->set_domain((uint8_t)ardomain);
...
template <typename CFG> inline void axi::pin::ace_target<CFG>::rresp_t() {
    ...
    while(true) {
        ...
        SCCTRACE(SCMOD)<<__FUNCTION__ <<" val = "<< val << " beat count = " <<fsm_hndl->beat_count ;
        SCCTRACE(SCMOD)<<__FUNCTION__ << " got read response beat of trans " << *fsm_hndl->trans;

        // hongyu replace wit ace signal
