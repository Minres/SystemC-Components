...
template <class T> inline in_place_t in_place(detail::in_place_type_tag<T> = detail::in_place_type_tag<T>()) { return in_place_t(); }

template <std::size_t K> inline in_place_t in_place(detail::in_place_index_tag<K> = detail::in_place_index_tag<K>()) {
    return in_place_t();
}

template <class T> inline in_place_t in_place_type(detail::in_place_type_tag<T> = detail::in_place_type_tag<T>()) { return in_place_t(); }

template <std::size_t K> inline in_place_t in_place_index(detail::in_place_index_tag<K> = detail::in_place_index_tag<K>()) {
    return in_place_t();
}
...
template <std::size_t K> struct index_tag_t {};

template <std::size_t K> inline void index_tag(index_tag_t<K> = index_tag_t<K>()) {}
...
struct is_swappable {
    template <typename T, typename = decltype(swap(std::declval<T&>(), std::declval<T&>()))> static std::true_type test(int);
...
struct is_nothrow_swappable {
    ...
    template <typename T> static constexpr bool test() { return noexcept(swap(std::declval<T&>(), std::declval<T&>())); }
...
template <typename T> struct is_nothrow_swappable : decltype(detail::is_nothrow_swappable::test<T>(0)) {};
...
template <class List, std::size_t CmpIndex, std::size_t LastChecked = typelist_size<List>::value> struct typelist_type_is_unique {
private:
...
public:
    enum V {
        value = ((CmpIndex == (LastChecked - 1)) | !std11::is_same<cmp_type, cur_type>::value) &&
                typelist_type_is_unique<List, CmpIndex, LastChecked - 1>::value
    };
};

template <class List, std::size_t CmpIndex> struct typelist_type_is_unique<List, CmpIndex, 0> {
    enum V { value = 1 };
};

template <class List, class T> struct typelist_contains_unique_type : typelist_type_is_unique<List, typelist_index_of<List, T>::value> {};
...
typedef variant_ALIGN_TYPE(char), variant_ALIGN_TYPE(short), variant_ALIGN_TYPE(int), variant_ALIGN_TYPE(long), variant_ALIGN_TYPE(float),
    variant_ALIGN_TYPE(double), variant_ALIGN_TYPE(long double),

    variant_ALIGN_TYPE(char*), variant_ALIGN_TYPE(short*), variant_ALIGN_TYPE(int*), variant_ALIGN_TYPE(long*), variant_ALIGN_TYPE(float*),
    variant_ALIGN_TYPE(double*), variant_ALIGN_TYPE(long double*),

    variant_ALIGN_TYPE(Unknown (*)(Unknown)), variant_ALIGN_TYPE(Unknown* Unknown::*), variant_ALIGN_TYPE(Unknown (Unknown::*)(Unknown)),
...
struct helper {
    ...
    template <class U> static U const* as(void const* data) { return reinterpret_cast<const U*>(data); }
    ...
    template <std::size_t K, class... Args> static type_index_t construct_i(void* data, Args&&... args) {
...
template <std::size_t K, class T0, class T1, class T2, class T3, class T4, class T5, class T6, class T7, class T8, class T9, class T10,
...
struct variant_alternative<K, variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15>> {
    typedef
        typename detail::typelist_type_at<variant_TL16(T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15), K>::type type;
};

#if variant_CPP11_OR_GREATER
template <std::size_t K, class T> using variant_alternative_t = typename variant_alternative<K, T>::type;
#endif
...
template <class T0, class T1 = detail::T1, class T2 = detail::T2, class T3 = detail::T3, class T4 = detail::T4, class T5 = detail::T5,
          class T6 = detail::T6, class T7 = detail::T7, class T8 = detail::T8, class T9 = detail::T9, class T10 = detail::T10,
          class T11 = detail::T11, class T12 = detail::T12, class T13 = detail::T13, class T14 = detail::T14, class T15 = detail::T15>
class variant {
...
public:
...
#if variant_CPP11_OR_GREATER
    template <variant_index_tag_t(0) = variant_index_tag(0) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 0>::value)>
    variant(T0 const& t0)
    : type_index(0) {
        new(ptr()) T0(t0);
    }

    template <variant_index_tag_t(1) = variant_index_tag(1) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 1>::value)>
    variant(T1 const& t1)
    : type_index(1) {
        new(ptr()) T1(t1);
    }

    template <variant_index_tag_t(2) = variant_index_tag(2) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 2>::value)>
    variant(T2 const& t2)
    : type_index(2) {
        new(ptr()) T2(t2);
    }

    template <variant_index_tag_t(3) = variant_index_tag(3) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 3>::value)>
    variant(T3 const& t3)
    : type_index(3) {
        new(ptr()) T3(t3);
    }

    template <variant_index_tag_t(4) = variant_index_tag(4) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 4>::value)>
    variant(T4 const& t4)
    : type_index(4) {
        new(ptr()) T4(t4);
    }

    template <variant_index_tag_t(5) = variant_index_tag(5) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 5>::value)>
    variant(T5 const& t5)
    : type_index(5) {
        new(ptr()) T5(t5);
    }

    template <variant_index_tag_t(6) = variant_index_tag(6) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 6>::value)>
    variant(T6 const& t6)
    : type_index(6) {
        new(ptr()) T6(t6);
    }

    template <variant_index_tag_t(7) = variant_index_tag(7) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 7>::value)>
    variant(T7 const& t7)
    : type_index(7) {
        new(ptr()) T7(t7);
    }

    template <variant_index_tag_t(8) = variant_index_tag(8) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 8>::value)>
    variant(T8 const& t8)
    : type_index(8) {
        new(ptr()) T8(t8);
    }

    template <variant_index_tag_t(9) = variant_index_tag(9) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 9>::value)>
    variant(T9 const& t9)
    : type_index(9) {
        new(ptr()) T9(t9);
    }

    template <variant_index_tag_t(10) = variant_index_tag(10) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 10>::value)>
    variant(T10 const& t10)
    : type_index(10) {
        new(ptr()) T10(t10);
    }

    template <variant_index_tag_t(11) = variant_index_tag(11) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 11>::value)>
    variant(T11 const& t11)
    : type_index(11) {
        new(ptr()) T11(t11);
    }

    template <variant_index_tag_t(12) = variant_index_tag(12) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 12>::value)>
    variant(T12 const& t12)
    : type_index(12) {
        new(ptr()) T12(t12);
    }

    template <variant_index_tag_t(13) = variant_index_tag(13) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 13>::value)>
    variant(T13 const& t13)
    : type_index(13) {
        new(ptr()) T13(t13);
    }

    template <variant_index_tag_t(14) = variant_index_tag(14) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 14>::value)>
    variant(T14 const& t14)
    : type_index(14) {
        new(ptr()) T14(t14);
    }

    template <variant_index_tag_t(15) = variant_index_tag(15) variant_REQUIRES_B(detail::typelist_type_is_unique<variant_types, 15>::value)>
        ...
        std::is_nothrow_move_constructible<T0>::value&& std::is_nothrow_move_constructible<T1>::value&&
            std::is_nothrow_move_constructible<T2>::value&& std::is_nothrow_move_constructible<T3>::value&&
                std::is_nothrow_move_constructible<T4>::value&& std::is_nothrow_move_constructible<T5>::value&&
                    std::is_nothrow_move_constructible<T6>::value&& std::is_nothrow_move_constructible<T7>::value&&
                        std::is_nothrow_move_constructible<T8>::value&& std::is_nothrow_move_constructible<T9>::value&&
                            std::is_nothrow_move_constructible<T10>::value&& std::is_nothrow_move_constructible<T11>::value&&
                                std::is_nothrow_move_constructible<T12>::value&& std::is_nothrow_move_constructible<T13>::value&&
                                    std::is_nothrow_move_constructible<T14>::value&& std::is_nothrow_move_constructible<T15>::value)
    : type_index(other.type_index) {
        (void)helper_type::move_construct(other.type_index, other.ptr(), ptr());
    }

    template <std::size_t K> using type_at_t = typename detail::typelist_type_at<variant_types, K>::type;
    ...
    template <class T, class U, class... Args variant_REQUIRES_T(std::is_constructible<T, std::initializer_list<U>&, Args...>::value)>
    ...
    template <std::size_t K, class... Args variant_REQUIRES_T(std::is_constructible<type_at_t<K>, Args...>::value)>
    ...
    template <size_t K, class U,
              class... Args variant_REQUIRES_T(std::is_constructible<type_at_t<K>, std::initializer_list<U>&, Args...>::value)>
            ...
            std::is_nothrow_move_assignable<T2>::value&& std::is_nothrow_move_assignable<T3>::value&&
                std::is_nothrow_move_assignable<T4>::value&& std::is_nothrow_move_assignable<T5>::value&&
                    std::is_nothrow_move_assignable<T6>::value&& std::is_nothrow_move_assignable<T7>::value&&
                        std::is_nothrow_move_assignable<T8>::value&& std::is_nothrow_move_assignable<T9>::value&&
                            std::is_nothrow_move_assignable<T10>::value&& std::is_nothrow_move_assignable<T11>::value&&
                                std::is_nothrow_move_assignable<T12>::value&& std::is_nothrow_move_assignable<T13>::value&&
                                    std::is_nothrow_move_assignable<T14>::value&& std::is_nothrow_move_assignable<T15>::value) {
    ...
    std::size_t index() const { return variant_npos_internal() == type_index ? variant_npos : static_cast<std::size_t>(type_index); }
...
#if variant_CPP11_OR_GREATER
        noexcept(
            std::is_nothrow_move_constructible<T0>::value&& std17::is_nothrow_swappable<T0>::value&& std::is_nothrow_move_constructible<
                T1>::value&& std17::is_nothrow_swappable<T1>::value&& std::is_nothrow_move_constructible<T2>::value&&
                std17::is_nothrow_swappable<T2>::value&& std::is_nothrow_move_constructible<T3>::value&& std17::is_nothrow_swappable<
                    T3>::value&& std::is_nothrow_move_constructible<T4>::value&& std17::is_nothrow_swappable<T4>::value&&
                    std::is_nothrow_move_constructible<T5>::value&& std17::is_nothrow_swappable<
                        T5>::value&& std::is_nothrow_move_constructible<T6>::value&& std17::is_nothrow_swappable<T6>::value&&
                        std::is_nothrow_move_constructible<T7>::value&& std17::is_nothrow_swappable<
                            T7>::value&& std::is_nothrow_move_constructible<T8>::value&& std17::is_nothrow_swappable<T8>::value&&
...
private:
    ...
    template <class U> static variant_constexpr std::size_t to_size_t(U index) { return static_cast<std::size_t>(index); }

    variant_constexpr type_index_t variant_npos_internal() const variant_noexcept { return static_cast<type_index_t>(-1); }
...
template <class T, class T0, class T1, class T2, class T3, class T4, class T5, class T6, class T7, class T8, class T9, class T10, class T11,
          class T12, class T13, class T14, class T15>
inline typename std11::add_pointer<T>::type get_if(variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15>* pv,
                                                   nonstd_lite_in_place_type_t(T) = nonstd_lite_in_place_type(T)) {
    return (pv->index() == variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15>::template index_of<T>())
               ? &get<T>(*pv)
               : variant_nullptr;
}
...
template <std::size_t K, class T0, class T1, class T2, class T3, class T4, class T5, class T6, class T7, class T8, class T9, class T10,
          class T11, class T12, class T13, class T14, class T15>
inline typename std11::add_pointer<
    typename variant_alternative<K, variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15>>::type>::type
...
template <std::size_t K, class T0, class T1, class T2, class T3, class T4, class T5, class T6, class T7, class T8, class T9, class T10,
          class T11, class T12, class T13, class T14, class T15>
inline typename std11::add_pointer<
    const typename variant_alternative<K, variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15>>::type>::type
get_if(variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15> const* pv,
       nonstd_lite_in_place_index_t(K) = nonstd_lite_in_place_index(K)) {
    return (pv->index() == K) ? &get<K>(*pv) : variant_nullptr;
}
          ...
          class T15
              ...
              variant_REQUIRES_T(
                  std::is_move_constructible<T0>::value&& std17::is_swappable<T0>::value&& std::is_move_constructible<
                      T1>::value&& std17::is_swappable<T1>::value&& std::is_move_constructible<T2>::value&& std17::is_swappable<T2>::value&&
                      std::is_move_constructible<T3>::value&& std17::is_swappable<T3>::value&& std::is_move_constructible<T4>::value&&
                          std17::is_swappable<T4>::value&& std::is_move_constructible<T5>::value&& std17::is_swappable<T5>::value&&
                              std::is_move_constructible<T6>::value&& std17::is_swappable<T6>::value&& std::is_move_constructible<
                                  T7>::value&& std17::is_swappable<T7>::value&& std::is_move_constructible<T8>::value&&
                                  std17::is_swappable<T8>::value&& std::is_move_constructible<T9>::value&& std17::is_swappable<T9>::value&&
                                      std::is_move_constructible<T10>::value&& std17::is_swappable<T10>::value&& std::is_move_constructible<
                                          T11>::value&& std17::is_swappable<T11>::value&& std::is_move_constructible<T12>::value&&
                                          std17::is_swappable<T12>::value&& std::is_move_constructible<T13>::value&& std17::is_swappable<
                                              T13>::value&& std::is_move_constructible<T14>::value&& std17::is_swappable<T14>::value&&
...
template <typename R, typename VT> struct VisitorApplicatorImpl {
    template <typename Visitor, typename T> static R apply(Visitor const& v, T const& arg) { return v(arg); }
};
...
template <typename R, typename Visitor, typename T2> struct TypedVisitorUnwrapper<2, R, Visitor, T2> {
...
template <typename R, typename Visitor, typename T2, typename T3> struct TypedVisitorUnwrapper<3, R, Visitor, T2, T3> {
    const Visitor& visitor;
    T2 const& val2;
    T3 const& val3;
...
template <typename R, typename Visitor, typename T2, typename T3, typename T4> struct TypedVisitorUnwrapper<4, R, Visitor, T2, T3, T4> {
    const Visitor& visitor;
    T2 const& val2;
    T3 const& val3;
    T4 const& val4;
    ...
    template <typename T> R operator()(const T& val1) const { return visitor(val1, val2, val3, val4); }
};
...
struct TypedVisitorUnwrapper<5, R, Visitor, T2, T3, T4, T5> {
    ...
    template <typename T> R operator()(const T& val1) const { return visitor(val1, val2, val3, val4, val5); }
};
...
template <typename R, typename Visitor, typename V2> struct VisitorUnwrapper {
    ...
    template <typename T1, typename T2, typename T3> R operator()(T1 const& val1, T2 const& val2, T3 const& val3) const {
...
template <typename R> struct VisitorApplicator {
    ...
    template <size_t Idx, typename Visitor, typename V1> static R apply_visitor(const Visitor& v, const V1& arg) {
    ...
    template <typename Visitor, typename V1, typename V2> static R apply(const Visitor& v, V1 const& arg1, V2 const& arg2) {
        typedef VisitorUnwrapper<R, Visitor, V1> Unwrapper;
        Unwrapper unwrapper(v, arg1);
...
template <typename R, typename Visitor, typename V1> inline R visit(const Visitor& v, V1 const& arg1) {
    return detail::VisitorApplicator<R>::apply(v, arg1);
}

template <typename R, typename Visitor, typename V1, typename V2> inline R visit(const Visitor& v, V1 const& arg1, V2 const& arg2) {
...
inline bool operator==(variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15> const& v,
    ...
    if(v.index() != w.index())
        ...
        return detail::Comparator<variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15>>::equal(v, w);
}
...
inline bool operator<(variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15> const& v,
    ...
    if(w.valueless_by_exception())
        ...
        return detail::Comparator<variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15>>::less_than(v, w);
}
...
struct hash<nonstd::variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15>> {
    std::size_t
    operator()(nonstd::variant<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14, T15> const& v) const variant_noexcept {
