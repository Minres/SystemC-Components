...
enum class lock_e: uint8_t {
    NORMAL   = 0x0,
    EXLUSIVE = 0x1,
    LOCKED   = 0x2
};

enum class resp_e: uint8_t {
    OKAY   = 0x0,
    EXOKAY = 0x1,
    SLVERR = 0x2,
    DECERR = 0x3
...
inline bool ahb_extension::is_instruction() const {
    return prot & INSTR;
}
...
inline bool ahb_extension::is_privileged() const {
    return prot & PRIV;
}
...
inline bool ahb_extension::is_bufferable() const {
    return prot & BUFFERABLE;
}

inline void ahb_extension::set_bufferable(bool bufferable) {
    if(bufferable) prot|=BUFFERABLE;
    else prot &= ~BUFFERABLE;
}

inline bool ahb_extension::is_cacheable() const {
    return prot & CACHABLE;
}

inline void ahb_extension::set_cacheable(bool cacheable) {
    if(cacheable) prot|=CACHABLE;
    else prot &= ~CACHABLE;
}

inline uint8_t ahb_extension::get_protection() const {
    return prot;
}

inline void ahb_extension::set_protection(uint8_t protection) {
    prot=protection;
}

inline bool ahb_extension::is_locked() const {
    return lock==lock_e::LOCKED;
}

inline void ahb_extension::set_locked(bool locked) {
    lock=locked?lock_e::LOCKED:lock_e::NORMAL;
}

inline uint8_t ahb_extension::get_burst() const {
    return static_cast<uint8_t>(burst);
}

inline burst_e ahb_extension::get_burst_e() const {
    return burst;
}
...
inline void ahb_extension::set_burst(burst_e b) {
    burst=b;
}
