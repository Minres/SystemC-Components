...
struct sc_variable_b : sc_core::sc_object {
    ...
    virtual void trace(observer* obs, const std::string& name ) const = 0;
    virtual void trace(sc_core::sc_trace_file* tf,const std::string& name) const = 0;
...
template <typename T> struct sc_variable : public sc_variable_b {
    ...
    // assignment operator overload
    sc_variable& operator=(const sc_variable other) {
        value = other.value;
        for(auto h : hndl)
            h->notify();
        return *this;
     ...
     * @param tf
     */
    void trace(sc_core::sc_trace_file* tf, const std::string& name) const override {
    ...
    void trace(observer* obs, const std::string& name) const override { hndl.push_back(observe(obs, value, name)); }
...
template <typename T> struct sc_ref_variable : public sc_variable_b {
     ...
     * @param tf
     */
    void trace(sc_core::sc_trace_file* tf, const std::string& name) const override { sc_core::sc_trace(tf, value, name); }

    void trace(observer* obs,const std::string& name ) const override {}
};
...
#if(defined SYSTEMC_2_3_3 || SYSTEMC_VERSION >= 20171012)
    void trace(sc_core::sc_trace_file* tf,const std::string& name) const override { sc_core::sc_trace(tf, value, name); }
#else
    void trace(sc_core::sc_trace_file* tf,const std::string& name) const override { }
#endif
...
template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_variable<T>& object, const std::string& name) {
    object.trace(tf, name);
}
template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_variable<T>* object, const std::string& name) {
    return sc_trace(tf,&object,name);
}

template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_ref_variable<T>& object, const std::string& name) {
    object.trace(tf,name);
}
template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_ref_variable<T>* object, const std::string& name) {
    return sc_trace(tf,&object,name);
...
template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_variable<T>& object, const std::string& name,int width ) {
    object.trace(tf, name);
}
template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_variable<T>* object, const std::string& name,int width) {
    return sc_trace(tf,&object,name,width);
}

template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_ref_variable<T>& object, const std::string& name,int width) {
    object.trace(tf, name);
}
template <class T> inline void sc_trace(sc_core::sc_trace_file* tf, const sc_ref_variable<T>* object, const std::string& name,int width) {
    return sc_trace(tf,&object,name,width);
...
template <class T> inline void sc_trace(sc_trace_file* tf, const ::scc::sc_variable<T>& object, const std::string& name) {
    object.trace(tf,name);
}
template <class T> inline void sc_trace(sc_trace_file* tf, const ::scc::sc_variable<T>* object, const std::string& name) {
    return sc_trace(tf,&object,name);
...
template <class T> inline void sc_trace(sc_trace_file* tf, const ::scc::sc_ref_variable<T>& object, const std::string& name) {
    object.trace(tf,name);
}
template <class T> inline void sc_trace(sc_trace_file* tf, const ::scc::sc_ref_variable<T>* object, const std::string& name) {
    return sc_trace(tf,&object,name);
...
template <class T> inline void sc_trace(sc_trace_file* tf, const ::scc::sc_variable<T>& object, const std::string& name,int width ) {
    return sc_trace(tf,object,name);
}
template <class T> inline void sc_trace(sc_trace_file* tf, const ::scc::sc_variable<T>* object, const std::string& name,int width) {
    return sc_trace(tf,&object,name);
...
template <class T> inline void sc_trace(sc_trace_file* tf, const ::scc::sc_ref_variable<T>& object, const std::string& name,int width) {
    return sc_trace(tf,object,name);
}
template <class T> inline void sc_trace(sc_trace_file* tf, const ::scc::sc_ref_variable<T>* object, const std::string& name,int width) {
    return sc_trace(tf,&object,name);
