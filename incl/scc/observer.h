...
struct observer {
    ...
    virtual void default_observe(a_bv_base const& o, std::string const& nm) = 0;
    ...
    virtual notification_handle* observe(a_bv_base const& o, std::string const& nm) = 0;
    ...
    inline void default_observe(observer* obs, a_bv_base const& o, std::string const& nm) {
        obs->default_observe(o, nm);
    }
...
DECL_REGISTER_METHOD_A(a_bv_base)
...
inline void sc_trace(sc_core::sc_trace_file* tf, const a_bv_base& object, const std::string& name) {
	if(auto* obs = dynamic_cast<observer*>(tf))
	{
		default_observe(obs,object,name);
	}
}
...
//DEFN_TRACE_FUNC_C( sc_variable_b)
...
namespace sc_core {
	...
	inline void sc_trace(sc_trace_file* tf, const a_bv_base& object, const std::string& name) {
		if(auto* obs = dynamic_cast<scc::observer*>(tf))
		{
			scc::default_observe(obs,object,name);
		}
	}
